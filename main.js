...
try {
                            ...
                            for (const pair of tickers) {
                                 console.log(`NOOUT'${pair.tradingPair.id}'|'${pair.price.toFixed(8)}'`);
                            }
...
//cipi end
		...
		console.log('Use COSSbot manually (m), automatic trading (a), or quit (q)?  ');
		var tradingMode=getline().trim();
        ...
        if (tradingMode === 'm') {
            ...
            postMessage("Manual trading mode initializing...\n")
                ...
                while (manualCommand !== 'b' || manualCommand !== 'back'){
					...
					console.log('Please select a command for COSSbot to execute. Type help (h) to see all options:  ');
					var manualCommand=getline().trim();
                    ...
                    //gets highest bid and lowest ask prices for specified pair
                        ...
                        //var pairSelected = readlineSync.question('Enter a pairing (currency-pairing) e.g. coss-eth:  ');
						 console.log('Enter a pairing (currency-pairing) e.g. coss-eth:  ');
						var pairSelected=getline().trim();
                    ...
                    //Place sell order (market or limit)   
                       ...
                       // var buyType = readlineSync.question('Would you like to place a limit sell (1) or market sell (2):  ');
						console.log('Would you like to place a limit sell (1) or market sell (2):  ');
						var buyType=getline().trim();
						...
						console.log('Enter a trading pair (currency-pairing) e.g. coss-eth:  ');
						var pairSelected=getline().trim();
        
                        //var sellPrice = readlineSync.question('Please enter your desired ask price per unit in pairing (e.g. ETH/BTC/FIAT):  ');
                         console.log('Please enter your desired ask price per unit in pairing (e.g. ETH/BTC/FIAT):  ');
						var sellPrice=getline().trim();
        
						//var amount = readlineSync.question('Please enter the amount you want to sell:  ');
						console.log('Please enter the amount you want to sell:  ');
						var amount=getline().trim();
                    ...
                    // place  buy
						...
						console.log('Would you like to place a limit buy (1) or market buy (2):  ');
						var buyType=getline().trim();
                        ...
                        //var buyPrice = readlineSync.question('Please enter the price of your bid price in (pairing, e.g. ETH/BTC/FIAT):  ');
                         console.log('Please enter the price of your bid price in (pairing, e.g. ETH/BTC/FIAT):  ');
						var buyPrice=getline().trim();
						//var amount = readlineSync.question('Please enter the amount you want to buy:  ');
                        console.log('Please enter the amount you want to buy:  ');
						var amount=getline().trim();
            ...
            function automatedTrading(){
				...
				console.log('Enter a pairing (currency-pairing) e.g. coss-eth for COSSbot to trade on:  ');
				var p4=getline().trim();
                ...
                console.log('What is the order size you want COSSbot to make in units of '+ currencyName+ ':  ') // number of COSS bought at each purchase (for both initial and averaging down);
				var p5=parseFloat(getline().trim());
				var p6 = '' 
                //var price = readlineSync.questionFloat('Please enter the current price of '+ currencyName+ ' in '+ pairingName+': ')
                console.log('Please enter the current price of '+ currencyName+ ' in '+ pairingName+': ');
				var price=parseFloat(getline().trim());
                ...
                console.log('At what percent would you like COSSbot to take profit (please enter as a percent, e.g. 5, 10, or 4.25, do not include % sign): ')
				var profitPercent=parseFloat(getline().trim());
                ...
                console.log('At what percent would you like COSSbot to take cost average down? (please enter as a percent, e.g. 5, 10, or 4.25, do not include % sign): ')
				var priceDrop=parseFloat(getline().trim());
